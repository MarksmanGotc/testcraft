...
function createScreenshotButton() {
  ...
  // … (nappi-luonti ennallaan) …

  button.addEventListener('click', () => {
    const target = document.querySelector('.wrapper');
    if (!window.html2canvas || !target) return;

    // 1) Piilota nappi ennen renderöintiä
    const prevDisplay = button.style.display;
    button.style.display = 'none';

    // 2) Mittaa elementti ja näytön DPI
    const rect = target.getBoundingClientRect();
    const dpr  = window.devicePixelRatio || 1;

    html2canvas(target, {
      // ota tarkka DPI huomioon
      scale: dpr,

      // rajaa juuri tuon elementin kokoon
      width:  rect.width,
      height: rect.height,
      x:      rect.left + window.scrollX,
      y:      rect.top  + window.scrollY,

      // poista taustaväri-pakotus, niin CSS:n oma tausta tulee mukaan
      backgroundColor: null,

      useCORS:  true,
      // foreignObjectRendering parantaa tekstin ja gradienttien renderöintiä
      foreignObjectRendering: true,
    }).then(canvas => {
      // 3) Näytä nappi takaisin ja lataa
