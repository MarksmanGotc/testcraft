...
function createCloseButton(parentElement) {
 ...
 function createScreenshotButton() {
   ...
   button.addEventListener('click', () => {
     ...
     html2canvas(target, {
      // rajoitetaan skaalaksi max. 2 → riittävä laatu, nopeampi
      scale: Math.min(dpr, 2),

      // varmista, että tausta ei jää läpinäkyväksi (joka katselussa voi tulla mustana)
      backgroundColor: window.getComputedStyle(target).backgroundColor || '#ffffff',

      useCORS:       true,
      // oletusrenderöinti on usein riittävän tarkka ja huomattavasti nopeampi
      foreignObjectRendering: false,

      // sammuta debug-logit ja lyhennä odotusaikaa
      logging:       false,
      imageTimeout:  1000,

      // rajaa juuri wrapperin kokoon
