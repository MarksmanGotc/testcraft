...
const SCARCITY_RATIO_PENALTY_BREAKPOINTS = Object.freeze([
    { ratio: 0.5, penalty: 0 },
    { ratio: 0.3, penalty: -5 },
    { ratio: 0.15, penalty: -10 },
    { ratio: 0.07, penalty: -15 },
    { ratio: 0, penalty: -20 }
]);
...
function getUserPreferences(availableMaterials) {
    ...
    const scarcityPenaltyByMaterial = {};
    ...
    if (sortedMaterials.length > 0) {
        ...
        const maxAmount = sortedMaterials[0][1];
        sortedMaterials.forEach(([materialName, amount]) => {
            if (amount === leastAmount) {
                normalizedLeastMaterials.add(normalizeKey(materialName));
                if (maxAmount > 0) {
                    const ratio = (Number(amount) || 0) / maxAmount;
                    const penalty = getScarcityPenaltyForRatio(ratio);
                    if (penalty !== 0) {
                        scarcityPenaltyByMaterial[normalizeKey(materialName)] = penalty;
                    }
                }
            }
        });
    ...
    return {
        ...
        sortedMaterials,
        scarcityPenaltyByMaterial
    };
}

function getScarcityPenaltyForRatio(ratio) {
    if (!Number.isFinite(ratio)) {
        return 0;
    }

    if (ratio <= 0) {
        return SCARCITY_RATIO_PENALTY_BREAKPOINTS[SCARCITY_RATIO_PENALTY_BREAKPOINTS.length - 1].penalty;
    }

    for (const { ratio: threshold, penalty } of SCARCITY_RATIO_PENALTY_BREAKPOINTS) {
        if (ratio >= threshold) {
            return penalty;
        }
    }
...
function getRankScore(rank, isLeastMaterial, scarcityPenalty = 0) {
    if (isLeastMaterial) {
        return -25 + Math.min(0, scarcityPenalty);
    }
...
function getMaterialScore(
    ...
    const availableMap = getNormalizedKeyMap(availableMaterials);
    const { rankByMaterial, leastMaterials, scarcityPenaltyByMaterial } = preferenceInfo || {};
    let totalPoints = 0;
    let materialTypes = 0;
    ...
    for (const [material, amountRequired] of Object.entries(product.materials)) {
        ...
        const scarcityPenalty =
            (scarcityPenaltyByMaterial &&
                scarcityPenaltyByMaterial[normalizedMatchedKey]) ||
            0;
        let materialScore = getRankScore(rank, isLeastMaterial, scarcityPenalty);
